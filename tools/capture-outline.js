window.__captureOutline = async function (mode) {
  ...
  let hulls = 0;
  p.model.traverse(o => { if (o.userData?.isOutline) { hulls++; if (mode === 'off') o.visible = false; } });
  ...
  const shots = [];
  for (const q of [{name:'game', pos:[0,10.3,11.4], look:[0,1.4,0]}, {name:'near', pos:[0,3.2,5.2], look:[0,1.1,0]}]) {
